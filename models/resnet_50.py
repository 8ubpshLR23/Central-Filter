...
class Bottleneck(nn.Module):
    def __init__(self, inplanes , midplanes, planes, compress_rate = 0., stride=1, is_downsample=False):
        ...
        # print(compress_rate)
        inplanes  = int(inplanes * (1-compress_rate[0]))
        midplanes_1 = int(midplanes * (1-compress_rate[1]))
        midplanes_2 = int(midplanes * (1-compress_rate[2]))
        planes    = int(planes * (1-compress_rate[3]))
...
class ResNet50(nn.Module):
    def __init__(self, compress_rate, num_classes=1000):
        ...
        layer_num = 0
        inplanes  = int(64 * (1-compress_rate[0]))
        ...
        self.fc = nn.Linear(int(2048*(1.-compress_rate[-1])), num_classes)
...
def resnet_50(compress_rate=None,oristate_dict = None,ranks = None):
    model = None 
    if oristate_dict is not None and compress_rate is not None: #and ranks is not None
