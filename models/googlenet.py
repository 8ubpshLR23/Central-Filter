...
class Inception(nn.Module):
    def __init__(self, in_planes, n1x1, n3x3red, n3x3, n5x5red, n5x5, pool_planes,tmp_name, compress_rate, last=False):
        ...
        if self.n3x3:
            ...
            if last:
                ...
                output=int(round(n3x3*(1.-compress_rate[0]),3))
        ...
        if self.n5x5 > 0:
            ...
            if last:
                ...
                output=int(round(n5x5*(1.-compress_rate[2]),3))

            conv5x5_1 = nn.Conv2d(in_planes, n5x5red, kernel_size=1)
            conv5x5_2 = nn.Conv2d(n5x5red, int(n5x5*(1.-compress_rate[1])), kernel_size=3, padding=1)
            conv5x5_3 = nn.Conv2d(int(n5x5*(1.-compress_rate[1])), output, kernel_size=3, padding=1)
            ...
            self.branch5x5 = nn.Sequential(
                ...
                nn.ReLU(True),
                conv5x5_2,
                nn.BatchNorm2d(int(n5x5*(1.-compress_rate[1]))),
                nn.ReLU(True),
                conv5x5_3,
                nn.BatchNorm2d(output),
                nn.ReLU(True),
...
class GoogLeNet(nn.Module):
    def __init__(self, compress_rate, block=Inception, filters=None):
        ...
        first_outplanes=int(192*(1.-compress_rate[0]))
        ...
        in_plane_list=[]
        for i in range(8):
            # print(i,compress_rate[i*3+1],compress_rate[i*3+2],compress_rate[i*3+3])
            in_plane_list.append(filters[i][0]+int(round(filters[i][1]*(1.-compress_rate[i*3+1]) ,3) )+int(round(filters[i][2]*(1.-compress_rate[i*3+3]) ,3))+filters[i][3])

        self.inception_a3 = block(first_outplanes, filters[0][0], mid_filters[0][0], filters[0][1], mid_filters[0][1], filters[0][2], filters[0][3], 'a3', compress_rate[1:3+1])
        self.inception_b3 = block(in_plane_list[0], filters[1][0], mid_filters[1][0], filters[1][1], mid_filters[1][1], filters[1][2], filters[1][3], 'a4', compress_rate[3+1:3*2+1])

        self.maxpool1 = nn.MaxPool2d(3, stride=2, padding=1)
        self.maxpool2 = nn.MaxPool2d(3, stride=2, padding=1)

        self.inception_a4 = block(in_plane_list[1], filters[2][0], mid_filters[2][0], filters[2][1], mid_filters[2][1], filters[2][2], filters[2][3], 'a4', compress_rate[3*2+1:3*3+1])
        self.inception_b4 = block(in_plane_list[2], filters[3][0], mid_filters[3][0], filters[3][1], mid_filters[3][1], filters[3][2], filters[3][3], 'b4', compress_rate[3*3+1:3*4+1])
        self.inception_c4 = block(in_plane_list[3], filters[4][0], mid_filters[4][0], filters[4][1], mid_filters[4][1], filters[4][2], filters[4][3], 'c4', compress_rate[3*4+1:3*5+1])
        self.inception_d4 = block(in_plane_list[4], filters[5][0], mid_filters[5][0], filters[5][1], mid_filters[5][1], filters[5][2], filters[5][3], 'd4', compress_rate[3*5+1:3*6+1])
        self.inception_e4 = block(in_plane_list[5], filters[6][0], mid_filters[6][0], filters[6][1], mid_filters[6][1], filters[6][2], filters[6][3], 'e4', compress_rate[3*6+1:3*7+1])

        self.inception_a5 = block(in_plane_list[6], filters[7][0], mid_filters[7][0], filters[7][1], mid_filters[7][1], filters[7][2], filters[7][3], 'a5', compress_rate[3*7+1:3*8+1])
        self.inception_b5 = block(in_plane_list[7], filters[8][0], mid_filters[8][0], filters[8][1], mid_filters[8][1], filters[8][2], filters[8][3], 'b5', compress_rate[3*8+1:3*9+1], last=True)
...
def googlenet(compress_rate = None,oristate_dict = None,ranks = None):
    ...
    if compress_rate is None :
        ...
        # _in_rank    = list(range(192))  # ignore the pre_layer
        _in_rank    = ranks[0][0]
        _rank       = []
        _3x3_rank   = ranks[1][0]
